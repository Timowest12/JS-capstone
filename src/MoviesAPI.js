...
const getmovielist = () => {
  ...
  .then(data => data.forEach((row) => {
      if (row.image != null) {
          console.log(row)
          moviesoutput.innerHTML += `<div class='moviecard'><div class='imgblock'><img class='movieimage' src=${row.image.medium}></div><div class='infoblock'><div class='likesandname'><h3 class='movietitle'>${row.name}</h3></div><div class='commentsandreservations'><button type='button'>comments</button><button type='button'>reservations</button></div></div></div>`;
      }
  }));
}
const getfromid = (id) => {
